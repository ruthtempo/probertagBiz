...
function App() {
  const [wohnungList, setWohnungList] = useState([]); //original retrieved list
  const [sortBy, setSortBy] = useState(); //sorting modus
  const [sortedWohnungList, setSortedWohnungList] = useState(); //copy after sorting
  ...
  useEffect(() => {
    ...
    if (sortBy === "price") {
      const sortedList = wohnungListCopy.sort((a, b) => a.price - b.price); //ascending
      setSortedWohnungList(sortedList);
    } else if (sortBy === "sqm") {
      const sortedListSqm = wohnungListCopy.sort((a, b) => b.sqm - a.sqm); //descending
